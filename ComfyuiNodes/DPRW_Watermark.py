...
class DPRWatermark:
    ...
    # def _create_watermark(self, total_blocks: int, message: str, message_length: int, LDPC_AVAILABLE = True) -> bytes:
    #     """先LDPC编码再重复：对消息进行LDPC编码增强纠错能力，然后重复填满容量"""
    #     # 确定实际的比特长度
    #     length_bits = message_length if message_length > 0 else choose_watermark_length(total_blocks)
        
    #     # 如果LDPC不可用，直接使用原始方法
    #     if not LDPC_AVAILABLE:
    #         self.logger.warning("[DPRWatermark]LDPC库未安装，使用原始重复编码")
    #         return self._create_watermark_original(total_blocks, message, message_length)
        
    #     try:
    #         # 计算单个LDPC块的大小，确保它不会太大也不会太小
    #         # 目标是每个LDPC块占总容量的10-25%
    #         target_block_ratio = 0.2  # 目标比例
    #         ldpc_block_bits = max(min(int(total_blocks * target_block_ratio), 2048), length_bits)
            
    #         # 调整ldpc_block_bits为32的倍数，便于LDPC参数设置
    #         ldpc_block_bits = (ldpc_block_bits // 32) * 32
            
    #         # LDPC参数，使用适中的码率
    #         d_v = 3  # 变量节点度
    #         d_c = 12  # 校验节点度，对应码率约为0.25
            
    #         # 调整区块大小以确保能被d_c整除
    #         ldpc_block_bits = ((ldpc_block_bits + d_c - 1) // d_c) * d_c
            
    #         self.logger.info(f"[DPRWatermark]Create watermark - LDPC块大小: {ldpc_block_bits}位")
            
    #         # 将消息转为字节并填充
    #         msg_bytes = message.encode('utf-8')
            
    #         # 计算该LDPC参数下可以容纳的信息位数量
    #         info_bits = ldpc_block_bits * d_v // d_c
    #         info_bytes = (info_bits // 8)
            
    #         # 填充/截断消息到适合的长度
    #         padded_msg = msg_bytes.ljust(info_bytes, b'\x00')[:info_bytes]
            
    #         self.logger.info(f"[DPRWatermark]Create watermark - 原始消息: '{message}'")
    #         self.logger.info(f"[DPRWatermark]Create watermark - 消息长度: {len(msg_bytes)*8}位 -> 填充至{info_bits}位")
    ...
    #         # 确保位数匹配
    #         if len(bit_array) < info_bits:
    #             bit_array.extend([0] * (info_bits - len(bit_array)))
    #         bit_array = np.array(bit_array[:info_bits])
            
    #         try:
    #             # 设置合理的LDPC参数 - 确保n > k
    #             d_v = 3  # 变量节点度
    #             d_c = 12  # 校验节点度，码率约为d_v/d_c = 0.25
                
    #             # 计算信息位数量k (LDPC能容纳的信息位)
    #             k = info_bits
                
    #             # 计算码字长度n，确保n > k且d_c整除n
    #             n = int(k * d_c / d_v)
    #             n = ((n + d_c - 1) // d_c) * d_c  # 确保n是d_c的倍数
                
    #             self.logger.info(f"[DPRWatermark]Create watermark - 计算LDPC参数: k={k}, n={n}, 码率={k/n:.4f}")
                
    #             # 创建LDPC码
    #             H, G = make_ldpc(n, d_v, d_c, systematic=True, seed=42)
                
    #             # 检查实际码率
    #             actual_k, actual_n = G.shape
    #             actual_rate = actual_k / actual_n
    #             self.logger.info(f"[DPRWatermark]Create watermark - 实际LDPC参数: k={actual_k}, n={actual_n}, 码率={actual_rate:.4f}")
                
    #             # 调整输入数据长度以匹配生成矩阵维度
    #             if len(bit_array) < actual_k:
    #                 bit_array = np.pad(bit_array, (0, actual_k - len(bit_array)))
    #             bit_array = bit_array[:actual_k]
                
    #             # LDPC编码
    #             encoded_bits = encode(G, bit_array, seed=42, snr=20)
                
    #             # 将编码后的位转回字节
    #             ldpc_block_bytes = bytearray()
    #             for i in range(0, len(encoded_bits), 8):
    #                 if i + 8 <= len(encoded_bits):
    #                     byte = 0
    #                     for j in range(8):
    #                         byte |= (int(encoded_bits[i + j]) << (7 - j))
    #                     ldpc_block_bytes.append(byte)
                
    #             # 计算LDPC块的字节长度
    #             ldpc_block_byte_length = (ldpc_block_bits + 7) // 8
                
    #             # 补足LDPC块到完整字节
    #             if len(ldpc_block_bytes) < ldpc_block_byte_length:
    #                 ldpc_block_bytes.extend([0] * (ldpc_block_byte_length - len(ldpc_block_bytes)))
                
    #             # 计算需要重复的次数
    #             total_bytes = total_blocks // 8
    #             repeats = total_bytes // len(ldpc_block_bytes)
                
    #             self.logger.info(f"[DPRWatermark]Create watermark - LDPC编码后长度: {len(encoded_bits)}位")
    #             self.logger.info(f"[DPRWatermark]Create watermark - LDPC块大小: {len(ldpc_block_bytes)}字节")
    #             self.logger.info(f"[DPRWatermark]Create watermark - 重复次数: {repeats}次")
                
    #             # 创建最终的水印：LDPC编码块的重复
    #             result = bytes(ldpc_block_bytes) * repeats
                
    #             # 处理可能的剩余空间
    #             remaining_bytes = total_bytes - len(result)
    #             if remaining_bytes > 0:
    #                 result += bytes(ldpc_block_bytes[:remaining_bytes])
                
    #             return result
                    
    #         except Exception as e:
    #             self.logger.warning(f"[DPRWatermark]LDPC编码失败: {e}: {traceback.format_exc()}")
    #             return self._create_watermark_original(total_blocks, message, message_length)
                
    #     except Exception as e:
    #         self.logger.warning(f"[DPRWatermark]LDPC处理失败，回退到重复编码: {e}: {traceback.format_exc()}")
    #         return self._create_watermark_original(total_blocks, message, message_length)
    ...
    def _create_watermark(self, total_blocks: int, message: str, message_length: int, BCH_AVAILABLE = False) -> bytes:
        ...
        try:
            ...
            # Determine a valid BCH_T (error correction capability in bits)
            # k_actual_lib = N - m*T, where m=8 for N=255
            # We need k_actual_lib >= actual_info_bits_payload
            # So, 255 - 8*T >= actual_info_bits_payload
            # 8*T <= 255 - actual_info_bits_payload
            # T <= (255 - actual_info_bits_payload) / 8
            max_t_for_payload = (BCH_N_BITS - actual_info_bits_payload) // 8
            
            # Choose a T. Let's try to maximize it, or use a reasonable fixed value.
            # The original code had BCH_T = 45, which was too high.
            # If actual_info_bits_payload = 48, max_t_for_payload = (255-48)//8 = 207//8 = 25
            ...
            # The library determines its k_bits based on N and m*T
            # k_lib_bits = bch.n_bits - bch.ecc_bits (n_bits is 255, ecc_bits is 8*BCH_T_CORRECTION_BITS)
            # k_lib_bytes = bch.k_bits // 8
            # The input data to bch.encode() must be k_lib_bytes long.
            ...
            if data_bytes != bch.k_bytes:
                ...
                # Option 1: Adjust data_bytes to bch.k_bytes (might change TARGET_INFO_BITS interpretation)
                # data_bytes = bch.k_bytes 
                # self.logger.info(f"[DPRWatermark] Adjusted data_bytes to {data_bytes} to match BCH lib requirements.")
                # Option 2: Error out or re-calculate T. The current setup (T derived from payload) should align.
                # If actual_info_bits_payload (e.g. 48) is used to derive T=25, then k_lib_bits should be 55.
                # bch.k_bytes for k_bits=55 is 55//8 = 6. This matches data_bytes=6 from TARGET_INFO_BITS=45.
                # This warning should ideally not trigger if T is derived correctly.
            ...
            # 校验编码后的长度是否正确
            # Expected length is bch.n_bits / 8
            # bch.n_bits should be 255. So, (255+7)//8 = 32 bytes if n_bits could be non-multiple of 8,
            # or bch.n_bytes if the library provides it (usually N/8 if N is multiple of 8, or ceil(N/8)).
            # For N=255, total bits = 255. Total bytes = (255+7)//8 = 32 bytes (since we must contain all 255 bits)
            
            # bch.n_bytes is often n_bits // 8, which for 255 would be 31.
            # This implies the last byte is partially filled.
            # len(bch_block_bytes) should be bch.k_bytes + bch.ecc_bytes = (55//8) + (200//8) = 6 + 25 = 31 bytes.
            bch_block_total_bits = bch.n_bits # Should be 255
            ...
            # The logic below for bch_block_bytes_expected was a bit confusing with BCH_N.
            # The actual total length in bytes is len(bch_block_bytes)
            # This should be (bch.n_bits + 7) // 8 if we need to store all bits in full bytes.
            # Or, if bchlib operates on N that isn't multiple of 8 by padding, then bch.n_bytes is key.
            # Assuming bch.n_bits = 255, then physical storage would need ceil(255/8) = 32 bytes.
            # However, bch.encode often produces k_bytes + ecc_bytes, which is (255-8T)//8 + (8T)//8 = (255//8) total bytes = 31 bytes.
            # This means the 255 bits are packed into 31 bytes, with the last few bits of the 31st byte unused.
            # Let's rely on the length of the actual combined block:
