...
def save_latent_data(noise: dict, target_name: str, save_dir: str = "./data_outputs"):
    """
    将 noise 数据保存为 JSON 格式
    ...
    try:
        ...
        filename = f"latent_data_{target_name}_{timestamp}.json"
        ...
        save_data = {
            "metadata": {
                "timestamp": timestamp,
                f"{target_name}_shape": list(noise["samples"].shape),
            },
            f"target_name": {
                "samples": noise["samples"].cpu().numpy().tolist(),
                "shape": list(noise["samples"].shape),
                "dtype": str(noise["samples"].dtype)
            },
        }
        ...
        print(f"成功保存潜在表示数据到: {filepath}")
        
    except Exception as e:
        print(f"保存潜在表示数据时出错: {str(e)}")
        traceback.print_exc()
...
def common_ksampler(model, seed: int, steps: int, cfg: float, sampler_name: str, scheduler: str, positive, negative, latent,
    ...
    out = latent.copy()
    out["samples"] = samples
    if noramWorkFlowFlag:
        save_latent_data(init_noise, "init_noise_ZT")
        save_latent_data(out, "init_noise_denoised_Z0")
    else:
        save_latent_data(init_noise, "init_noise_Z0_watermarked")
        # 这里得到的就是x0，因为xN的混合过程是在comfyui内完成的
        save_latent_data(out, "init_noise_denoised_x0_watermarked")
...
class DPRLatentSaver:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "latent_image": ("LATENT",),
                "latent_name": ("STRING", {"default": "noise"}),
                "save_dir": ("STRING", {"default": "./data_outputs", "multiline": False,}),
            }
        }
    ...
    def save_latent(self, latent_image, latent_name, save_dir="./data_outputs"):
        print(f"Saving latent image with name: {latent_name} to directory: {save_dir}")
        save_latent_data(latent_image, latent_name, save_dir)
...
class DPR_Blend_Latents:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "latent_a": ("LATENT",),
                "latent_b": ("LATENT",),
                "operation": (["dprw", "add", "multiply", "divide", "subtract", "overlay", "hard_light", "soft_light", "screen", "linear_dodge", "difference", "exclusion", "random"],),
    ...
    def blend_latents(self, latent1, latent2, mode='add', blend_percentage=0.5):
        ...
        def DPRW_blend(latent1, latent2, blend_factor):
            if isinstance(latent1, torch.Tensor):
                latent1 = latent1.cpu()
            if isinstance(latent2, torch.Tensor):
                latent2 = latent2.cpu()
                
            blended_latent = (latent1 * (1 - blend_factor)) + (latent2 * blend_factor)
        ...
        if mode == 'add':
            ...
            blended_latent = DPRW_blend(latent1, latent2, blend_factor1)
        else:
